...
def load_models(n_steps=3,n_features=6):# dinamik yaz
    ...
    for coin in config["Coins"]:
        ...
        print(glob.glob("/home/shoujakymn/Desktop/Reguler Code/checkpoints/{}_4h_macd*".format(coin)))
        ...
        for c,i in enumerate(currencies):
            try:
                ...
                if abs(current[i]["Close"][-1]/real_last_price[c]-1) > 0.0083 and (int(current[i]["Close"][-1]/real_last_price[c])!=flag_c[c] and flag_lose[c]==0 and flag_win[c]==0):
                    flag_lose[c]=1
                if abs(current[i]["Close"][-1]/real_last_price[c]-1) > 0.0249 and (int(current[i]["Close"][-1]/real_last_price[c])==flag_c[c] and flag_lose[c]==0 and flag_win[c]==0):
                    flag_win[c]=1

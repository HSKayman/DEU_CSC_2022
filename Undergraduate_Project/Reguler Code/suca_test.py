...
while (int(time.time()) - 3600) % 60 < 55:   
    pass
...
# =============================================================================
    ...
    flag_currency=[[0.0249,0.0249,0.0249] for i in currencies] #1 alt limit 0 ust limit 2 adding
    print("Waiting till prediction time comes!")
    while (int(time.time()) - 3600) % 60 >55:   
        pass
    while (int(time.time()) - 3600) % 60 <55:
        ...
        for c,i in enumerate(currencies):
            try:
                ...
                data=requests.get(url)
                data=data.json()
                current[i]["Close"][-1]=float(input('amount: '))#float(data['price'])
                if abs(current[i]["Close"][-1]/real_last_price[c]-1) > 0.0083 and (int(current[i]["Close"][-1]/real_last_price[c])!=flag_c[c] and flag_lose[c]==0 and flag_win[c]==0):
                    flag_lose[c]=1
                #abs(current[i]["Close"][-1]/real_last_price[c]-1) > flag_currency[c][0] and 
                if (int(current[i]["Close"][-1]/real_last_price[c])==flag_c[c] and flag_lose[c]==0 and flag_win[c]==0):
                    if abs(current[i]["Close"][-1]/real_last_price[c]-1) > flag_currency[c][0]:
                        flag_currency[c][2]/=2
                        adding = flag_currency[c][2]
                        flag_currency[c][1]=flag_currency[c][0]
                        flag_currency[c][0]+=adding
                    elif abs(current[i]["Close"][-1]/real_last_price[c]-1) <= flag_currency[c][1]:
                        flag_win[c]=1
                print(abs(current[i]["Close"][-1]/real_last_price[c]),'->','\n alt limit:',flag_currency[c][1]+1,'\nust limit:',flag_currency[c][0]+1)    
...
# =============================================================================
    ...
    while j<len(currencies):  
        ...
        if flag_win[c]==1:
            roulete[j]=roulete[j]*flag_currency[j][1]
            dictionary[currencies[j]+"_PorL"].append("profit")
            print(currencies[j],".coin profit: {}".format(flag_currency[j][1]),"->",real_now_price[j])
            if int((real_now_price[c]/real_last_price[c]))!=flag_c[c]:
                print("Take profit working")
                confs[j][3-3*flag_c[j]]+=flag_currency[j][1]*10
            elif real_profit[j]>0.0249:
                print("Take profit fakeout")
                confs[j][1+flag_c[j]]+=flag_currency[j][1]*15
        elif flag_lose[j]==1:
            roulete[j]=roulete[j]*0.9917
            dictionary[currencies[j]+"_PorL"].append("lose")
            print(currencies[j],".coin lose: 0.0083","->",real_now_price[j])
            if int((real_now_price[c]/real_last_price[c]))==flag_c[c]:
                print("Stop loss fakeout")
                confs[j][1+flag_c[j]]+=0.0083*15
            elif real_profit[j]>0.0083:
                print("Stop loss working")
                confs[j][1+flag_c[j]]+=0.0083*5

...
# =============================================================================
    ...
    last_max_loss=[0.0253 for i in currencies]
        ...
        for c,i in enumerate(currencies):
            ...
            if abs(current[i]["Close"][-1]/real_last_price[c]-1) >last_max_profit[c] and (int(current[i]["Close"][-1]/real_last_price[c])==flag_c[c]) and flag_win[c]==0:
                ...
                if last_max_profit[c] < 0.01:
                    ...
                    last_max_loss[c] = 0.0253 / ((last_max_profit[c]*100) + 1)
                ...
                if abs(current[i]["Close"][-1]/real_last_price[c]-1) <= float( last_max_profit[c] )- float(last_max_loss[c]):
                    flag_win[c]= float( last_max_profit[c] )- float(last_max_loss[c])
                ...
                if abs(current[i]["Close"][-1]/real_last_price[c]-1) >= abs(float( last_max_profit[c] )- float(last_max_loss[c])) :
                    flag_win[c]=float( last_max_profit[c] )- float(last_max_loss[c])
                    '''print(flag_win[c],'Your profit')
                    print(last_max_profit[c],'Your max profit')
                    print(last_max_loss[c],'Your stop loss')
